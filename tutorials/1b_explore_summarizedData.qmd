...
Notice that there are `r nrow(summarizedData)` rows - each row here corresponds to a cell line-drug combination. Making up these combinations are 288 unique cell lines, and 15 drugs.
...
Now we'll summarize different measures of correlation of the IC50 values to assess the level of replication between these two experiments. First, we'll compute the two different measures for correlating continous variables mentioned above (Pearson and Spearman correlation coefficients).
...
Next, we'll visualize the correlations the IC50 measurements in a grouped bar plot. To do this, we'll first have the reshape the data using the `gather` function from the tidyverse.
