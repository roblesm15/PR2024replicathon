...
If you have any question about the code, ask one of the mentors. Also remember that [Google](https://www.google.com) search and [ChatGPT](https://chatgpt.com) can aid you in data science tasks.
...
There are [several pre-defined themes for plotting with `ggplot2`](https://ggplot2.tidyverse.org/reference/ggtheme.html). While the default "`theme_gray`" is nice, we will set the default to "`theme_bw`" using the `theme_set` function.
